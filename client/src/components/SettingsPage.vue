        ...
        <div class="data-management">
          <div class="data-import-section">
            ...
            <div class="import-actions">
              <div class="import-info">
                <p><strong>适用场景：</strong>初次使用系统时批量导入产品基础信息</p>
                <p>支持 CSV 格式文件。条码字段请留空，导入后在"条码管理"页面批量生成。</p>
          ...
          <div class="data-export-section">
            ...
            <div class="export-info">
              <p><strong>适用场景：</strong>数据备份、Excel分析、系统迁移</p>
            </div>
          ...
          <div class="data-initialization-section">
            ...
            <div class="init-info">
              <p><strong>功能说明：</strong>将现有产品库存记录为历史入库交易，为月度台账提供正确的期初库存基础。</p>
              <p><strong>使用场景：</strong></p>
              <ul>
                <li>✓ 首次使用系统，已导入产品数据后</li>
                <li>✓ 系统重新部署后需要恢复历史状态</li>
                <li>✓ 需要为月度账本提供期初数据</li>
              </ul>
              <p><strong>注意事项：</strong></p>
              <ul>
                <li>• 初始化日期固定为 2025-07-01</li>
                <li>• 不会重新计算价格，使用产品现有的总价值</li>
                <li>• 每个系统建议只执行一次</li>
                <li>• 执行前请确保产品数据已正确导入</li>
              </ul>
            </div>
        ...
        <div class="backup-section">
          ...
          <div class="backup-database-section">
            ...
            <div class="backup-info">
              ...
              <p class="backup-format-info">
                <strong>备份格式：</strong>
                <br>• <strong>ZIP格式</strong>（推荐）：包含所有数据和单据图片文件，完整备份
                <br>• <strong>JSON格式</strong>：仅包含数据库数据，不含图片文件，体积较小
              </p>
            </div>
...
export default {
  ...
  setup() {
    ...
    const settingsTabs = ref([
      ...
      { id: 'data', name: '产品导入导出', icon: '📊' },
      { id: 'security', name: '安全设置', icon: '🔒' },
      { id: 'backup', name: '系统备份', icon: '💾' },
    ...
    const downloadBackup = async () => {
      try {
        ...
        const format = confirm(
          '请选择备份格式:\n\n' +
          '确定: ZIP格式（包含所有数据和图片文件）\n' +
          '取消: JSON格式（仅包含数据库数据，不含图片）\n\n' +
          '推荐使用ZIP格式以完整备份所有内容。'
        ) ? 'zip' : 'json'
        
        showToast(`正在生成${format.toUpperCase()}格式备份...`, 'info')
        
        if (format === 'zip') {
          // ZIP格式备份（包含图片）
          const response = await settingsAPI.exportData(null) // 使用默认参数
          ...
          // 调用导出接口获取ZIP文件
          const zipResponse = await api.get(`/settings/export?${params.toString()}`, {
            responseType: 'blob'
          })
          ...
          link.href = url
          link.download = `backup_full_${new Date().toISOString().split('T')[0]}.zip`
          ...
          showToast('ZIP备份下载成功（包含所有数据和图片）', 'success')
          
        } else {
          // JSON格式备份（仅数据）
          const response = await settingsAPI.exportData(['products', 'categories', 'projects', 'transactions', 'documents', 'snapshots', 'admin_settings'])
          ...
          showToast('JSON备份下载成功（不包含图片文件）', 'success')
        }
      } catch (error) {
    ...
    const restoreBackup = () => {
      ...
      input.onchange = async (e) => {
        ...
        if (file) {
          ...
          if (!isZip && !isJson) {
            showToast('只支持 .json 或 .zip 格式的备份文件', 'error')
            return
          }
          ...
          if (isZip) {
            ...
            try {
              if (!confirm(
                ...
                `⚠️ 警告：此操作将覆盖所有现有数据和图片文件！\n` +
                `建议在恢复前先下载当前数据的备份。`
              )) {
                return
              }
              
              showToast('正在恢复ZIP备份，请稍候...', 'info')
              ...
              // 调用后端API恢复ZIP备份
              const response = await settingsAPI.restoreZipBackup(formData)
              
              if (response.data.stats) {
                const restoredStats = response.data.stats
                const message = [
                  `✅ ZIP备份恢复成功！`,
                  ``,
                  `已恢复数据：`,
                  `• 产品: ${restoredStats.products || 0} 条`,
                  `• 类别: ${restoredStats.categories || 0} 条`,
                  `• 部门: ${restoredStats.projects || 0} 条`,
                  `• 交易记录: ${restoredStats.transactions || 0} 条`,
                  `• 单据: ${restoredStats.documents || 0} 条`,
                  `• 快照: ${restoredStats.snapshots || 0} 条`,
                  `• 图片文件: ${restoredStats.images || 0} 个`,
                  ``,
                  `页面即将自动刷新...`
                ].join('\n')
              ...
              showToast('备份恢复失败: ' + (error.response?.data?.error || error.message), 'error')
            }
            ...
            reader.onload = async (e) => {
              try {
                ...
                // 验证备份数据格式
                if (!backupData.data) {
                  showToast('备份文件格式错误：缺少data字段', 'error')
                  return
                }
                
                // 显示备份信息
                const info = backupData.backup_info || {}
                const stats = [
                  `产品: ${backupData.data.products?.length || 0} 条`,
                  `类别: ${backupData.data.categories?.length || 0} 条`,
                  `部门: ${backupData.data.projects?.length || 0} 条`,
                  `交易: ${backupData.data.transactions?.length || 0} 条`,
                  `单据: ${backupData.data.documents?.length || 0} 条`,
                  `快照: ${backupData.data.snapshots?.length || 0} 条`
                ].join('\n')
                ...
                if (!confirm(
                  ...
                  `备份信息：\n` +
                  `版本: ${info.version || '未知'}\n` +
                  `日期: ${info.created_at ? new Date(info.created_at).toLocaleString('zh-CN') : '未知'}\n\n` +
                  `数据量：\n${stats}\n\n` +
                  `⚠️ 警告：此操作将覆盖所有现有数据！\n` +
                  `⚠️ 注意：JSON备份不包含图片文件！\n` +
                  `建议在恢复前先下载当前数据的备份。`
                ...
                showToast('正在恢复备份，请稍候...', 'info')
                
                // 调用后端 API 恢复备份
                const response = await settingsAPI.restoreBackup(backupData)
    ...
    const resetSystem = async () => {
      if (confirm('⚠️ 警告：这将删除所有产品、交易记录、单据和快照数据，但会保留部门和产品类别。确定要继续吗？')) {
        if (confirm('这是最后确认，重置后数据无法恢复。确定要重置系统吗？')) {
          try {
            ...
            const message = [
              ...
              ``,
              `已删除：`,
              `• 产品: ${result.productsDeleted || 0} 条`,
              `• 交易记录: ${result.transactionsDeleted || 0} 条`,
              `• 单据: ${result.documentsDeleted || 0} 条`,
              `• 快照: ${result.snapshotsDeleted || 0} 条`,
              ``,
              `✓ 部门和产品类别已保留`
            ].join('\n')
    ...
    const executeImport = async () => {
      ...
      try {
        ...
        for (let i = 0; i < validItems.length; i++) {
          ...
          if (item.category_name) {
            if (categoryMap.has(item.category_name)) {
              ...
              try {
                ...
                const createdCategory = await inventoryStore.createCategory(newCategoryData)
                categoryMap.set(item.category_name, createdCategory.id)
                category_id = createdCategory.id
              } catch (error) {
                console.error(`Failed to create category "${item.category_name}":`, error)
                showToast(`创建类别"${item.category_name}"失败: ${error.message}`, 'error')
                return
              }
            }
          }
          ...
          // 只有当total_cost_value有值时才设置，避免设为0导致后端误判
          if (item.total_cost_value !== undefined && item.total_cost_value !== null && item.total_cost_value !== '') {
            productData.total_cost_value = item.total_cost_value
          }
          ...
          if (item.existing_id) {
            ...
            try {
              ...
              if (error.response && error.response.status === 400 && 
                  error.response.data.error && error.response.data.error.includes('已存在')) {
                console.log(`Product "${item.name}" already exists, attempting to find and update it`)
                try {
                  // 重新加载产品列表以获取最新数据
                  await inventoryStore.loadProducts()
                  const existingProduct = inventoryStore.products.find(p => 
                    p.name.trim().toLowerCase() === item.name.trim().toLowerCase()
                  )
                  if (existingProduct) {
                    console.log(`Found existing product for update: ${existingProduct.id}`)
                    await inventoryStore.updateProduct(existingProduct.id, productData)
                    updateCount++
                    console.log(`Successfully updated existing product "${item.name}"`)
                  } else {
                    console.error(`Could not find existing product "${item.name}" for update`)
                    failedCount++
                    failedItems.push(item.name)
                  }
                } catch (updateError) {
                  console.error(`Failed to update existing product "${item.name}":`, updateError)
                  failedCount++
                  failedItems.push(item.name)
                }
              } else {
                console.error(`Create product error for "${item.name}":`, error)
                failedCount++
                failedItems.push(item.name)
              }
            }
        ...
        const details = []
        if (successCount > 0) details.push(`新增 ${successCount} 条`)
        if (updateCount > 0) details.push(`更新 ${updateCount} 条`)
        if (failedCount > 0) details.push(`失败 ${failedCount} 条`)
        
        if (details.length > 0) {
          successMessage = `导入完成！${details.join('，')}产品`
        }
        
        if (failedItems.length > 0) {
          console.warn('导入失败的产品:', failedItems)
          successMessage += `\n注意：以下产品导入失败：${failedItems.join('、')}`
        }
