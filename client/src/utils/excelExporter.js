...
export class LedgerExcelExporter {
  ...
  createSummarySheet() {
    ...
    const summaryData = [
      ...
      ['总出入库金额', `¥${summary.total_transaction_amount.toFixed(4).replace(/\.?0+$/, '')}`],
      ['期末货值', `¥${summary.total_stock_value.toFixed(4).replace(/\.?0+$/, '')}`],
    ...
    categories.forEach(category => {
      ...
      summaryData.push([
        ...
        `¥${category.total_amount.toFixed(4).replace(/\.?0+$/, '')}`,
        `¥${category.category_stock_value.toFixed(4).replace(/\.?0+$/, '')}`,
        status
      ]);
    });
  ...
  async createCategoryDetailSheet(category) {
    ...
    // 类别标题
    categoryData.push([
      `${category.name} (${category.total_products_count}个产品，出入库总次数: ${category.products.reduce((sum, p) => sum + p.transaction_count, 0)}，出入库总额: ¥${category.total_amount.toFixed(4).replace(/\.?0+$/, '')}，仓库货值: ¥${category.category_stock_value.toFixed(4).replace(/\.?0+$/, '')})`
    ]);
    ...
    for (const product of category.products) {
      ...
      categoryData.push([
        ...
        `¥${product.total_amount.toFixed(4).replace(/\.?0+$/, '')}`,
        product.beginning_stock || 0,
        `¥${(product.beginning_stock_value || 0).toFixed(4).replace(/\.?0+$/, '')}`,
        product.ending_stock,
        `¥${product.ending_unit_price.toFixed(4).replace(/\.?0+$/, '')}`,
        `¥${product.ending_stock_value.toFixed(4).replace(/\.?0+$/, '')}`,
        '查看详情'
      ]);
        ...
        if (transactions.length > 0) {
          transactions.forEach((transaction) => {
            ...
            categoryData.push([
              ...
              `¥${transaction.unit_price.toFixed(4).replace(/\.?0+$/, '')}`,
              `¥${(transaction.quantity * transaction.unit_price).toFixed(4).replace(/\.?0+$/, '')}`,
              transaction.requester_name,
              transaction.project_name || '',
              transaction.purpose || '',
              transaction.stock_after || '',
              transaction.stock_unit_price ? `¥${transaction.stock_unit_price.toFixed(4).replace(/\.?0+$/, '')}` : '',
              transaction.stock_value ? `¥${transaction.stock_value.toFixed(4).replace(/\.?0+$/, '')}` : ''
            ]);
          });
        } else {
          categoryData.push([
