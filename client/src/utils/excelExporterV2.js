...
export class LedgerExcelExporterV2 {
  constructor(ledgerData, currentMonth) {
    ...
    this.workbook = new ExcelJS.Workbook();
         ...
         this.formatCurrency(product.total_amount),
        this.formatQuantity(product.beginning_stock),
        this.formatCurrency(product.beginning_stock_value || 0),
        this.formatQuantity(product.ending_stock),
        this.formatCurrency(product.ending_unit_price),
        this.formatCurrency(product.ending_stock_value),设置工作簿属性
  ...
  createSummarySheet() {
    ...
    const summaryData = [
      ...
      ['总出入库金额', this.formatCurrency(summary.total_transaction_amount)],
      ['期末货值', this.formatCurrency(summary.total_stock_value)],
    ...
    categories.forEach(category => {
      ...
      const row = worksheet.addRow([
        ...
        this.formatCurrency(category.total_amount),
        this.formatCurrency(category.category_stock_value),
        status
      ]);
  ...
  async createCategoryDetailSheet(category) {
    ...
    const titleRow = worksheet.addRow([
      `${category.name} (${category.total_products_count}个产品，出入库总次数: ${category.products.reduce((sum, p) => sum + p.transaction_count, 0)}，出入库总额: ${this.formatCurrency(category.total_amount)}，仓库货值: ${this.formatCurrency(category.category_stock_value)})`
    ]);
    ...
    for (const product of category.products) {
      ...
      const dataRow = worksheet.addRow([
        ...
        this.formatQuantity(product.ending_stock),
        this.formatCurrency(product.ending_unit_price),
        this.formatCurrency(product.ending_stock_value),
        '查看详情'
      ]);
        ...
        if (transactions.length > 0) {
          transactions.forEach((transaction) => {
            ...
            const transactionRow = worksheet.addRow([
              ...
              this.formatCurrency(transaction.unit_price),
              this.formatCurrency(transaction.quantity * transaction.unit_price),
              transaction.requester_name,
              transaction.project_name || '',
              transaction.purpose || '',
              this.formatQuantity(transaction.stock_after || 0),
              transaction.stock_unit_price ? this.formatCurrency(transaction.stock_unit_price) : '',
              transaction.stock_value ? this.formatCurrency(transaction.stock_value) : ''
            ]);
