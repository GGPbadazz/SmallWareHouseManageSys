...
export class LedgerExcelExporterV2 {
  ...
  createSummarySheet() {
    ...
    const summaryData = [
      ...
      ['总出入库金额', this.formatRawValue(summary.total_transaction_amount)],
      ['期末货值', this.formatRawValue(summary.total_stock_value)],
    ...
    categories.forEach(category => {
      ...
      const row = worksheet.addRow([
        ...
        this.formatRawValue(category.total_amount),
        this.formatRawValue(category.category_stock_value),
        status
      ]);
  ...
  async createCategoryDetailSheet(category) {
    ...
    // 设置列宽
    worksheet.columns = [
      { width: 15 }, { width: 18 }, { width: 18 }, { width: 12 }, { width: 15 }, { width: 12 },
      { width: 15 }, { width: 12 }, { width: 12 }, { width: 15 }, { width: 12 }, { width: 15 }
    ];

    // 类别标题
    const titleRow = worksheet.addRow([
      `${category.name} (${category.total_products_count}个产品，出入库总次数: ${category.products.reduce((sum, p) => sum + p.transaction_count, 0)}，出入库总额: ¥${this.formatRawValue(category.total_amount)}，仓库货值: ¥${this.formatRawValue(category.category_stock_value)})`
    ]);
    ...
    for (const product of category.products) {
      ...
      const dataRow = worksheet.addRow([
        ...
        this.formatRawValue(product.total_amount),
        this.formatQuantity(product.beginning_stock || 0),
        this.formatRawValue(product.beginning_stock_value || 0),
        this.formatQuantity(product.ending_stock),
        this.formatRawValue(product.ending_unit_price),
        this.formatRawValue(product.ending_stock_value),
        '查看详情'
      ]);
      ...
      worksheet.mergeCells(`A${transactionTitleRow.number}:L${transactionTitleRow.number}`);
      ...
      const transactionHeaderRow = worksheet.addRow([
        '  类型', '  单号', '  操作时间', '  数量', '  单价', '  总价', '  领料人', 
        '  单位', '  用途', '  库存', '  库存单价', '  库存价值'
      ]);
      
      transactionHeaderRow.eachCell((cell, colNumber) => {
        cell.style = {
          font: { bold: true, color: { argb: 'FF444444' }, size: 10 },
          fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFE8E8E8' } },
          alignment: { horizontal: 'center', vertical: 'middle', indent: 1 },
          border: this.getThinBorder()
        };
      });
      ...
      try {
        ...
        if (transactions.length > 0) {
          transactions.forEach((transaction) => {
            ...
            // 入库显示供应商，出库显示领用部门
            const unitInfo = transaction.type === 'IN' 
            ...
            const transactionRow = worksheet.addRow([
              ...
              `  ${this.formatRawValue(transaction.unit_price)}`,
              `  ${this.formatRawValue(transaction.total_price)}`,
              `  ${transaction.requester_name || '-'}`,
              `  ${unitInfo}`,
              `  ${transaction.purpose || '-'}`,
              `  ${this.formatQuantity(transaction.stock_after || 0)}`,
              transaction.stock_unit_price ? `  ${this.formatRawValue(transaction.stock_unit_price)}` : '  ',
              transaction.stock_value ? `  ${this.formatRawValue(transaction.stock_value)}` : '  '
            ]);
            
            // 根据交易类型设置行颜色 - 弱化颜色
            const isInbound = transactionType === '入库';
            const bgColor = isInbound ? 'FFF0F8F0' : 'FFFFF5F5'; // 更淡的绿色和红色
            const fontColor = isInbound ? 'FF2D5016' : 'FF8B0000'; // 更深的文字颜色
            
            transactionRow.eachCell((cell, colNumber) => {
              cell.style = {
                font: { color: { argb: fontColor }, bold: false, size: 9 },
                fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: bgColor } },
                alignment: { horizontal: 'left', vertical: 'middle', indent: 1 },
                border: this.getThinBorder()
              };
          ...
          noTransactionRow.eachCell((cell, colNumber) => {
            cell.style = {
              font: { color: { argb: 'FF999999' }, italic: true, size: 9 },
