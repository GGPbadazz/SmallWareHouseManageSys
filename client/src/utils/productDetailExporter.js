...
export class ProductDetailExporter {
  ...
  createProductDetailSheet() {
    ...
    const statsRow = worksheet.addRow([
      `统计期间: ${this.period} | 条码: ${this.product.barcode || '-'} | 类别: ${this.product.category_name || '-'} | 当前库存: ${this.formatQuantity(this.product.current_stock)} | 库存单价: ${this.formatRawValue(this.product.current_unit_price || 0)} | 库存价值: ${this.formatRawValue(this.product.total_cost_value || 0)}`,
      '', '', '', '', '', '', '', '', '', '', ''
    ]);
    ...
    this.transactions.forEach((transaction, index) => {
      const row = worksheet.addRow([
        ...
        this.formatRawValue(transaction.unit_price || 0),
        this.formatRawValue(transaction.total_price || 0),
        transaction.requester_name || '-',
        transaction.project_name || '-',
        transaction.purpose || '-',
        this.formatQuantity(transaction.stock_after || 0),
        this.formatRawValue(transaction.stock_unit_price || 0),
        this.formatRawValue(transaction.stock_value || 0)
      ]);
    ...
    if (this.transactions.length > 0) {
      ...
      const totalOutboundValue = this.transactions
        ...
        `入库金额: ${this.formatRawValue(totalInboundValue)}`,
        `出库金额: ${this.formatRawValue(totalOutboundValue)}`,
        '', '', '', ''
      ]);
