...
router.get('/stats', (req, res) => {
    try {
        ...
        // Calculate total inventory value (simplified - assuming unit price if available)
        const totalValue = db.prepare('SELECT SUM(stock) as total FROM products').get().total || 0;
        
        res.json({
            total_products: totalProducts,
            low_stock_count: lowStockCount,
            today_transactions: todayTransactions,
            total_value: totalValue * 100 // Simplified calculation
        });
    } catch (error) {
...
router.get('/inventory-status', (req, res) => {
    try {
        ...
        const totalStockValue = db.prepare('SELECT SUM(stock) as total FROM products').get().total || 0;
